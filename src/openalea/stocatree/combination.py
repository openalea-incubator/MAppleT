...
class Partit(object):
    def __init__(self, min=None, max=None, grade=None):
        ...
        self.partition_list = [0] * int(pow(2, self.grade)+1)
...
class Comb(object):
    def __init__(self, ranges=None, grade=None):
        ...
        for i in range(len(self.ranges)):
            self.points.append(Partit(min=self.ranges[i][0], max=self.ranges[i][1], grade=self.grade).partition_list)
